...
python main.py --check-missed-results # EDGAR backstop: alert when a Tier 1/2 name filed an 8-K Item 2.02 but Finnhub has no actuals yet (FIVE-class silent miss)
...
- **EDGAR results backstop (`--check-missed-results`, added 2026-06-04).** Results detection is Finnhub-only, so when Finnhub lags or parks an event on the wrong date a Tier 1/2 name reports and the agent stays silent (FIVE: reported 2026-06-03, Finnhub stuck at 2026-06-02 with no actuals). `run_edgar_results_fallback()` scans Tier 1/2 events that are overdue (`event_date <= today`, within `_MISSED_RESULTS_LOOKBACK_DAYS=10`), `reported=0`, no actuals, and probes SEC EDGAR (`find_earnings_release_filing`) for an 8-K Item 2.02. A hit ⇒ the company reported but Finnhub hasn't ingested it ⇒ loud `:rotating_light:` alert to #earnings with the filing link. Alert-only (beat/miss figures still come from Finnhub on a later sweep via the daily 14-day look-back); dedup'd per ticker+quarter in `kv_store` (`missed_results_alerted:TICKER:QUARTER`). Wired into `post_earnings_check.yml` after the Finnhub sweeps (`continue-on-error`, best-effort). See `feedback_no_silent_failures.md`.
