...
- **Calendar artifact persistence + branch scoping — `scripts/ci_restore_db_artifact.sh` (2026-09-08).** All four restoring workflows call one script that asks the artifacts API for `earnings-db`, takes the **newest unexpired artifact on main by `created_at`**, and downloads it by id. Selection is deterministic: there is no run enumeration and therefore nothing to select wrongly. Upload stays guarded by `if: github.ref == 'refs/heads/main'`, and `EA_DB_BOOTSTRAPPED=true` still makes a missing artifact post-bootstrap a Slack alert + workflow failure rather than a silent fresh-DB run.
  ...
  - **The guard gained a season-independent staleness check at the same time**, because its original trigger could not see this. Past-due-events-without-actuals is season dependent: on 2026-09-08, off-season, the rolled-back database measured **2** against a threshold of 40 and the guard correctly reported "healthy". The new check refuses a database whose newest `events.updated_at` is more than `EA_DB_MAX_CONTENT_AGE_HOURS` (default 24) old. Measured over the 30 most recent artifacts, the gap between upload time and content watermark was ≤0.01h for runs that write events, 0.4–4.96h for runs that only restore and re-upload, and **89.53h** for the rolled-back one — so 24h sits ~4.8x above the largest legitimate gap and ~3.7x below the defect.
