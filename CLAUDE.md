...
- **Tier 2** — universe in `Healthcare Services` or `MedTech` sectors (sector alone), **plus `Biopharma` names with `Core=Y`** (`TIER_2_CORE_ONLY_SECTORS`, `8760e9d`, 2026-08-06 — Core-gated because Biopharma is ~687 universe rows of which only ~38 are Core, and sector alone would enrol every clinical-stage shell), excluding Tier 1. Gets Calendar + TickTick.
...
- **An event can now close WITHOUT reporting — `closed_reason` (v13, 2026-08-07).** `reported = 0` was the entire definition of "still waiting to happen", which stops being true the moment the universe churns: a company that has been **acquired** can never report, so its event sits unreported forever. Measured 2026-08-06: **51 of 160** past-dated unreported rows belonged to tickers that had left coverage — APLS→Biogen, EXAS→Abbott, FOLD→BioMarin, CFLT→IBM, SEMR→Adobe — the oldest **98 days**. Nothing flagged them, because every alerting loop skips a ticker absent from `coverage_map`. That guard is right on its own terms (dead names would otherwise alert forever) but its side effect is that **leaving the universe stops the only lane that would notice.**
  ...
  - **The terminal state reaches TickTick and Slack, but NOT Google Calendar yet — a known, deliberate gap.** TickTick: the reconcile **sees** closed rows and **completes** their task (never deletes — the tick is the durable record, and a deleted task gets recreated by the next sync as though nothing happened). ⚠ **Do not "fix" that by filtering closed rows out of the reconcile — that was tried and it is worse than no filter at all**, because an already-created task is then never completed, never cleared, never retitled; it just stays overdue nagging about a company that no longer exists. The **creation** query in `main.py` is the one that must not see them. Slack: `list_open_questions` filters the terminal state, because a late `lock <date>` reply otherwise reaches `_apply_edgar_auto_correction`, which moves the calendar entry and **inserts a replacement row** (default `closed_reason = NULL`) before deleting the closed one — resurrecting the event, date-locked, possibly at a future date. **Calendar is the gap:** a closed event's calendar entry still renders as an ordinary "Earnings Release / Date passed (results pending)", so the documented source of truth for *published* state keeps asserting pending. Left unbuilt on purpose — every calendar mutation goes through the create-first `_move_calendar_event` chokepoint and the events in question are past-dated history, so the fix is low-harm and non-trivial in equal measure. Fix it when a *future*-dated close becomes possible, which today it is not.
  - **`close_departed_events` only touches STRICTLY past-dated rows with no actuals** (`event_date < today`, not `<=` — the sync runs at 6/7 AM and 2/3 PM ET, when a BMO name's actuals may not be in and an AMC name has not reported at all, so `<=` would terminally close a live same-day event; on the real database that is the difference between 54 and 53 closed), and **refuses an empty coverage set**. A ticker can leave the universe for reasons unrelated to the company ending (a Position change, a tier edit, a sector reclassification), so a *future* event on such a name may be perfectly real. And an empty universe means the exports read failed — closing every open event on that basis is a silent mass deletion of workflow state, the 2026-07-26 BOM incident all over again. It is wired into `run()` **after** `_assert_coverage_not_collapsed`, so a broken read aborts before it can do damage.
- **`events.tier` is re-derived from coverage every run — `restamp_tiers_from_coverage` (2026-08-12).** `tier` is stamped by `upsert_event`, which only touches a row when a provider re-emits that ticker+date in the fetch window. Providers re-emit an unpredictable subset each run (**123 of ~850** in-window rows on 2026-08-11), so a row whose date is already settled can carry a classification from months ago while coverage says otherwise. Every consumer gates on `tier <= 2`, so a row frozen at 3 is invisible to TickTick, the calendar, digest detail, cross-check and the consensus preview — and nothing reports it, because the row looks fine. Measured the day CM flagged 31 large-cap biopharma names `Core=Y`: **37 open 3Q26 rows** (LLY, AMGN, GILD, VRTX, ARGX, BBIO…) at tier 3 with no task and no path to one. **FUTURE-dated rows only** (`event_date >= today`), and that bound is load-bearing both ways: forward is where a stale tier still costs a task/calendar entry/alert, and restamping backwards would both silently restate finished seasons (`compute_season_stats` counts `quarter = ? AND tier <= 2`) and *demote* past rows — 3 of them on the 2026-08-12 snapshot (CAT, CVNA, ACVA), the last with a live TickTick task the reconcile would then stop seeing, stranding it overdue forever. Skips closed rows; **refuses an empty coverage map** (a failed read would otherwise demote the universe out of Tier 1/2 — the 2026-07-26 BOM blast radius via a different mechanism); ignores tickers absent from the map (`close_departed_events` owns that case and *closes* rather than demotes). Wired into `run()` **after** `_assert_coverage_not_collapsed` and **before** the TickTick creation query — both orderings pinned by source-scanning tests, because a row promoted here must get its task in the same run. Regressions: `test_held_names_are_tier_1.py`.
- **Schema is at v14.** `storage.py CURRENT_SCHEMA_VERSION=14`. Migrations are non-destructive. Fresh-DB `CREATE TABLE` duplicates v2–v12 columns; when adding a column, update both the migration and the fresh-DB statement. The `test_fresh_db_schema_matches_migration_path` test in `test_dedup.py` enforces this invariant. Column history: v3=`unseen_run_count`, v4=`date_locked`, v5=`last_xcheck_yf_dates`, v6=`date_confirmed`, v7=backfill for v6, v8=`announcement_url`, v9=`slack_thread_ts` + `slack_question_kind` + `slack_last_reply_ts` + `question_state` + `question_snooze_until` + `question_first_seen` + `kv_store` table, v10=`slack_channel_id`, v11=`event_hour_yf` (yfinance-inferred hour), v12=`call_datetime_utc`+`call_source` (conference-call timestamp), v13=`closed_reason`+`closed_at` (terminal state for events that can never report), v14=`consensus_snapshot` TABLE (pre-print FY Street consensus, board #298 Phase A — a new table, so its one `CREATE TABLE` in `_MIGRATIONS[14]` is also what the fresh-DB path runs; `test_consensus_snapshot.py::test_schema_v14_fresh_and_migrated_paths_agree`).
...
- **Calendar artifact persistence + branch scoping — `scripts/ci_restore_db_artifact.sh` (2026-09-08).** All four restoring workflows call one script that asks the artifacts API for `earnings-db`, takes the **newest unexpired artifact on main by `created_at`**, and downloads it by id. Selection is deterministic: there is no run enumeration and therefore nothing to select wrongly. Upload stays guarded by `if: github.ref == 'refs/heads/main'`, and `EA_DB_BOOTSTRAPPED=true` still makes a missing artifact post-bootstrap a Slack alert + workflow failure rather than a silent fresh-DB run.
  - **It replaced `dawidd6/action-download-artifact@v6`, which was picking stale artifacts.** That action chose by ENUMERATING RUNS (`search_artifacts` + `workflow_search` + `workflow_conclusion: success`), a heuristic over a paged cross-workflow run list. Measured 2026-09-08 across the twelve most recent runs of the four workflows: **two restored a stale artifact.** `reconcile_calendar` run `34259752925` restored a 2026-09-05 snapshot, skipping one from 2026-09-08 15:06, and re-uploaded it as the newest `earnings-db` — so the next daily run restored a database 3.6 days old in good faith. Content lost against the skipped artifact: events 2987→2980, `eps_actual` 1744→1740, `reported` 1752→1748, `kv_store` 70→69. Same mechanism as the 28-day August rollback, still live, just smaller. `actions/download-artifact@v4` remains unusable here (same-run artifacts only).
  - **The two-step restore+retry pair is gone**; the script retries the listing and the download itself, verifies the SQLite file (`quick_check`, `events` present and non-empty) BEFORE overwriting the working copy, and **distinguishes an unreachable API from an absent artifact** — conflating them is what turns an outage into a fresh-DB run. No `continue-on-error`.
  - **Selection is by upload time on purpose; CONTENT is `ci_db_rollback_guard.sh`'s job.** Two layers, two signals: this script guarantees "newest", the guard adjudicates "good" (and still ranks self-heal candidates by accumulated actuals, never by upload time). `EA_DB_RESTORE_ARTIFACT_ID` stays owned by the guard alone.
  - **The guard gained a season-independent staleness check at the same time**, because its original trigger could not see this. Past-due-events-without-actuals is season dependent: on 2026-09-08, off-season, the rolled-back database measured **2** against a threshold of 40 and the guard correctly reported "healthy". ~~The new check refuses a database whose newest `events.updated_at` is more than `EA_DB_MAX_CONTENT_AGE_HOURS` (default 24) old.~~ **SUPERSEDED 2026-09-14 (`40d4b85`): that age check became the outage** (it ran before the write, so once tripped nothing could advance the watermark — 3 days down between seasons). The guard now asks whether any recent artifact holds MORE accumulated actuals than the copy about to be written (actuals only accumulate, so a decrease is a rollback; tolerance 0), heals and continues rather than failing, and logs content age as **advisory only**. Measured over the 30 most recent artifacts, the gap between upload time and content watermark was ≤0.01h for runs that write events, 0.4–4.96h for runs that only restore and re-upload, and **89.53h** for the rolled-back one — so 24h sits ~4.8x above the largest legitimate gap and ~3.7x below the defect.
  - **Regressions: `test_ci_db_restore.py`** (8 tests, `gh` stubbed on PATH so they run offline). The selection tests feed the listing **oldest-first** deliberately — a selector that trusts API ordering fails them. All five mutations were checked: dropping the sort, reversing it, dropping the expired filter, dropping the branch filter, and removing the staleness check each fail at least one test. ⚠ The first version of these tests skipped five of eight silently because it shelled out to `jq`, which is not on PATH on this machine; the filtering and sorting were moved out of the `gh --jq` program into the shell **so that they could be tested at all**, and the stub now emits the flat projection instead. `gh api` has no `--arg`; `--raw-field` looks like the answer and is not (it turns the call into a POST, and returned `{"message":"Not Found"}` against the live API).
...
- **Task facets are TAGS, not title text — sector, Position, and `Reported` (2026-08-12).** JP: *"lets tag entries as portfolio or researching so I can sort by that"* and *"Instead of having [Reported] at the front as a text lets just add it as a tag that the stock has reported."* `mark_task_reported` no longer prefixes the title; it attaches `REPORTED_TAG`. `position_tag()` covers **all five** Position values, not just the two JP named — `Ready to Buy` / `Ready to Short` share the Core Watchlist list with Portfolio and Researching, so tagging only two would leave untagged rows in the middle of the sort he asked for. Tags are always **ensured (merged), never assigned**: `update_task_content(ensure_tags=[...])` merges against the just-read task, because `main.py`'s `notify_results` calls `mark_task_reported` with no tags at all and an assignment would strip sector+Position off every task that path touches.
  - **⚠ TickTick LOWERCASES tags.** Probe-verified 2026-08-12: POSTing `tags: ["MixedCase", "Portfolio"]` reads back `["portfolio", "mixedcase"]`. (An earlier note in this file claimed tags were "case-preserved" — that was wrong.) `_merge_tags` compares case-insensitively; a case-sensitive check would find `Portfolio` "missing" from `['portfolio']` on every one of the 3x/day reconcile passes and rewrite every managed task forever. It also dedupes **within its own arguments** — `mark_task_reported` appends `REPORTED_TAG` to a list the reconcile may already have put it in, which produced `['Reported', 'Healthcare Services', 'Reported']` until a test caught it.
  - **Legacy `[REPORTED] ` titles migrate lazily and idempotently.** `task_is_reported()` reads the tag OR the legacy prefix — the fallback matters: without it a pre-2026-08-12 task takes the mark-reported branch, which rewrites the body and re-counts it as newly reported on every pass. With it, the task falls to the cheap title-repair branch, where the clean title is now the expected form, so one write strips the prefix and adds the tag together. `_ticker_from_task_title` still strips the prefix.
  - **The reconcile only reaches its window** (14d back / 45d forward, `tier <= 2`, row present in DB), so a convention change needs a sweep: `scripts/migrate_ticktick_tags.py` (default `--quarters 2Q26,3Q26`; one POST per task rather than `update_task_content`'s read-then-write, which halves the call cost and doubles the safe pace). Measured before the sweep: **224** tasks still prefixed, **0 of 1209** tagged. Older lists are deliberately left alone — they carry no prefix at all, so they are internally consistent.
- **The one-day move rides in the TITLE, and the reconcile must not strip it (board #297, 2026-09-08).** JP: *"Append the stock's one-day post-earnings move to the end of the TickTick entry header ... The size of the move is how JP decides which calls are worth reading."* It already went into the task BODY (`Stock reaction: +5.2%`), which requires opening the task — a title is what is visible in the list. `build_task_title(..., move_pct=)` appends ` +5.2%`; `mark_task_reported` passes it through.
  - ⛑ **THE MOVE IS IN NO DATABASE COLUMN.** It is computed live from yfinance when the result lands (`main.py` passes `r.move.move_pct`); `events` has 36 columns and none of them is a move. So `reconcile_ticktick_tasks` — which runs 3x a day off the DB and deliberately touches no price feed — **cannot rebuild a title carrying one**.
  - That is why `strip_move_suffix` / `move_suffix_of` exist. The reconcile compares titles with the suffix stripped and **carries the suffix across any rewrite**. Without both halves every reported task reads as title-stale, the reconcile writes the move back off, the next result run puts it back, and the two fight for ever at three writes a day — silently.
  - **An unconfirmed row never carries a move**, for the same reason it never carries a date. **`+0.0%` is emitted rather than omitted**: a rounded-away move is a measurement, and "no move recorded" is a different fact.
  - ⚠ **A format the stripper cannot parse is permanent churn**, so the round trip is parametrised over every shape the builder emits. Mutation-checked: deleting the preservation left **all 498 other tests green** — only `test_ticktick_reconcile.py::test_the_reconcile_does_not_strip_the_move_from_a_reported_title`, which drives the real reconcile, catches it. Testing the two helpers proved nothing about the caller.
...
- **`--reconcile-ticktick` — TickTick tasks are a projection of DB truth (2026-07-22, committed `3324f47`; CI writes enabled 2026-07-24).** `sync_ticktick_tasks` only CREATES tasks and never edits, so a name whose projected date was wrong (or whose task pointer was destroyed by `upsert_event`'s same-quarter DELETE) drifted forever — the UNH 2Q26 failure (reported Jul 16, task stuck "Jul 27", never `[REPORTED]`). Three-part fix: `upsert_event` carries `ticktick_task_id` across the same-quarter DELETE (keeping the `reported=0` guard — ICLR); `notify_results` falls back to `find_task_pointer_for_quarter` + logs a WARNING; `reconcile_ticktick_tasks(conn, today)` is the durable idempotent backstop — keyed on (ticker, reporting-quarter), task identity from a live per-quarter list scan, corrects title+dates for CONFIRMED rows and strips dates from UNCONFIRMED ones (never body pre-report — see the `date_confirmed` safeguard bullet for the no-date-until-confirmed rule), marks reported rows `[REPORTED]`, never touches a completed (`status==2`) task. **Staleness guard (REWORKED 2026-09-17):** refuses to run if the `last_sync_completed` heartbeat in `kv_store` is >4d old (a frozen DB would revert correct dates). It used to key on the newest unreported-target `updated_at`, and that is the wrong question: `updated_at` moves only when a row's DATA changes, so between seasons the correct, unchanging October rows age while the DB is refreshed daily — the guard cannot tell *the sync ran and found nothing to change* from *this DB is frozen*. It blocked the reconcile for four days (2026-09-14..09-17) over 110 in-window rows newest-stamped 2026-09-10, with the database as a whole written 2026-09-16. `storage.mark_sync_completed` stamps the heartbeat at the end of `run()`, **gated on a non-empty fetch** so an upstream outage cannot forge freshness. A DB with no heartbeat falls back to the old row check, so a pre-2026-09-17 artifact is still caught. Mutation-checked four ways (ignore the heartbeat / don't abort on a stale one / drop the fallback / stamp unconditionally), each killing exactly one test; a per-row stale skip guards confirmed rows' date pushes (unconfirmed rows are date-stripped regardless of freshness — removal pushes nothing untrusted; ENSG 2Q26: Finnhub stopped returning the name 2026-06-22 and its task froze at an unmarked, wrong "Jul 22"). Manual: `python main.py --reconcile-ticktick --dry-run`.
...
- `storage.py` — SQLite schema (v13) + non-destructive migrations + upsert/lock helpers + `OPEN_EVENT_SQL` (the single definition of an open event) + `close_departed_events` + `restamp_tiers_from_coverage`.
...
- `scripts/backfill_ticktick_quarter.py` — one-shot: mint tasks for named tickers whose call already happened (the forward-only creation query can never reach them). Requires explicit `--tickers`; no "all" mode.
